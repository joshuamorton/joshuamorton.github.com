...
# terrifying. "Metaclass" should send shivers down your spine. So lets have
# some fun and pretend we're writing python in python, and now we need to
...
# collection of names mapped to objects. A dictionary is incredibly good at
# this. In fact, if your object does not take advantage of `__slots__`, a python
# object is really just a dictionary with some additional attachments.
...
# inheritance. Its possible to also support multiple inheritance, and I'll
# tackle that at the end. For now though, just single inheritance.
...
# Now we can build out a few convenience methods for creating a class (this
# would be the analog of `type` in python) and for instantiating a class (the
# analog of the implicit parts of `__new__` that you can't change in python).
# But first, we need to do something special and a tad hacky.
...
# class. Instead, we have to bind them to the individual instances of an object
# to be able to call methods with a familiar syntax (`obj.method(args)` instead
# of `cls.method(obj, args)`). Maybe I'll eventually address that.
...
# Now we need some way to support inheritance. So lets define a method to
# declare a type. Because defining every class manually is awful and unfun.
...
# `type` as a metaclass. Note that this will overwrite functions and values on
# the parent class, which is exactly the functionality we want. We also want to
# test this to make sure everything is in good working order.
...
# This looks a bit different than the way that classes are normally
# instantiated, but let's take a look at `type`. It is defined as
# `class type(name, bases, dict)`. Which means that on occasion, people will
# define classes using this function.
...
# So that's single inheritance object oriented types in 20 lines of code.
# Pretty cool. But python supports multiple inheritance, and even java does now.
# Mixins are awesome. How do we support that? The change is surprisingly small,
# that said I'm going to do it differently than python and model it after the
# way java 8 supports mixins.

# The change we need to make to our previous function is actually rather simple:
# instead of a single parent, we can have multiple parents. And unlike in
# python, the parents are unordered. This way, there's no need to define a
# method resolution order. Additionally, we steal from java the rule that any
# time there is a method conflict, ie. multiple parents implement a single
# method, the child class must reimplment that method.
...
# method conflicts. All great things. Just one tiny issue: I now need to
# redefine `_str` on ever class I define that inherits from multiple base
# classes. That is annoying and antithetical to the goals of object oriented
# programming. It's an easy fix though.
...
# And there's Java's type system. Easy. Python's is a bit more complex because
# of the builtin resolution of method order. Now just to be sure, here are some
# tests of the two new class multi methods
